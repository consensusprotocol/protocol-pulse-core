"""
X Engagement Sentry — end-to-end cycle for Protocol Pulse replies (V5 overhaul).

Responsibilities:
- Load command_center_config.json for monitored accounts and thresholds
- Fetch new tweets via XClient
- Store in XInboxTweet
- Generate drafts via x_reply_writer using V5 unified prompt
- Enforce 20-min spacing, 18/day cap, no same-author-twice-per-day
...
try:
    from core.config.response_prompt import MAX_REPLIES_PER_DAY, MIN_REPLY_SPACING_MINUTES
except Exception:
...
def _daily_draft_count() -> int:
    """Count drafts created today (UTC)."""
    try:
        today_start = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
        return XReplyDraft.query.filter(XReplyDraft.created_at >= today_start).count()
    except Exception:
        return 0


def _last_draft_time() -> datetime | None:
    """Return the creation time of the most recent draft."""
    try:
        latest = XReplyDraft.query.order_by(XReplyDraft.created_at.desc()).first()
        return latest.created_at if latest else None
    except Exception:
        return None


def _authors_replied_today() -> set:
    """Return set of author handles we've already drafted replies to today."""
    try:
        today_start = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
        today_drafts = (
            XReplyDraft.query
            .join(XInboxTweet, XReplyDraft.inbox_id == XInboxTweet.id)
            .filter(XReplyDraft.created_at >= today_start)
            .with_entities(XInboxTweet.author_handle)
            .all()
        )
        return {row[0].lower() for row in today_drafts if row[0]}
    except Exception:
        return set()


def _generate_drafts() -> int:
    """Generate drafts for new tweets using V5 unified prompt."""
    thresholds = _get_thresholds()
    queue_min = float(thresholds.get("queue_for_approval", 0.70))

    # --- V5 Overhaul: daily cap ---
    daily_count = _daily_draft_count()
    if daily_count >= MAX_REPLIES_PER_DAY:
        logger.info("X Sentry: daily cap reached (%d/%d), skipping generation.", daily_count, MAX_REPLIES_PER_DAY)
        return 0

    # --- V5 Overhaul: 20-min spacing ---
    last_time = _last_draft_time()
    if last_time:
        elapsed = (datetime.utcnow() - last_time).total_seconds() / 60
        if elapsed < MIN_REPLY_SPACING_MINUTES:
            logger.info("X Sentry: spacing enforced (%.1f min < %d min), skipping.", elapsed, MIN_REPLY_SPACING_MINUTES)
            return 0

    # --- V5 Overhaul: no same author twice per day ---
    replied_authors = _authors_replied_today()
    ...
    created = 0
    remaining = MAX_REPLIES_PER_DAY - daily_count

    for inbox in new_tweets:
        if created >= remaining:
            logger.info("X Sentry: hit remaining daily cap during batch.")
            break

        # Skip if we already replied to this author today
        if inbox.author_handle and inbox.author_handle.lower() in replied_authors:
            inbox.status = "skipped"
            db.session.add(inbox)
            logger.info("X Sentry: skipping @%s — already replied today.", inbox.author_handle)
            continue
        ...
        try:
            ...
            if result.get("skip"):
                inbox.status = "skipped"
                db.session.add(inbox)
                logger.info("X Sentry: skipped @%s — reason: %s", inbox.author_handle, result.get("reason", "unknown"))
                continue
            ...
            db.session.add(inbox)
            created += 1
            replied_authors.add((inbox.author_handle or "").lower())
        except Exception as e:  # pragma: no cover
