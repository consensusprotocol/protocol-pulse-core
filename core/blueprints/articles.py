...
@articles_bp.route("/api/v2/articles")
def api_articles():
    """
    Paginated JSON API — used by Load More and client-side search.
    Params: page, per_page, category, q (search), sort
    """
    from app import db
    from models import Article
    ...
    try:
        ...
        per_page = min(50, request.args.get("per_page", 24, type=int))
        ...
        def _to_dict(a):
            ...
            return {
                ...
                "url":              f"/article/{article_make_slug(a)}",
                "is_featured":      bool(getattr(a, "featured", False)),
            }

        return jsonify({
            "articles":    [_to_dict(a) for a in paged.items],
            "page":        page,
            "per_page":    per_page,
            "total":       paged.total,
        ...
        logger.error("api_articles error: %s", err)
