...
- **Hardcoded Values**: Library books and learning paths are hardcoded (lines 324-415 in `media_unified.html`), which
...
The frontend appears professional but has a critical spec vio
...
- **CLIP timing bug / broken timeline:** For `host == "CLIP"`, it stores `duration: clip_dur` but does not increment `current_time` and does not add any placeholder audio to concat (`292–303`). Every subsequent line starts too earl
