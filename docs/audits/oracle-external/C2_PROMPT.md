...
- **Locking Phone During Polling:** If the user locks their phone during polling, iOS Safari will suspend JavaScript execution after a brief grace period (usually 30 seconds). This means the polling loop will pause, and no further `fetch()` calls will be made until the phone is unlo
...
### U3 — Multiple Redundant State Variables Create Desynchroniz
