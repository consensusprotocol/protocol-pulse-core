...
- **Rate Limiting Gaps:** `Flask-Limiter` is set up with a default of "200 per day" (line 96), which applies
...
##### Hard spec conflict
- Governing law says **no Canvas** for UI animations/effects; CSS/SVG only. This file uses canvas for sparklines and gauge (`169-199`, `
...
**What to change:** Audit the HTML element IDs against the JS writers. Update `updateSignalStrength(
