...
-   **Potential API Key Exhaustion:** The TTS scripts retry API calls up to 3 times on failure (`tts_engine.py:210`). If the API key is invalid or the account quota is fully exhausted, the pipeline will still attempt to generate audio for every single line of a scrip
...
### U4 — Canvas elements violate technology
