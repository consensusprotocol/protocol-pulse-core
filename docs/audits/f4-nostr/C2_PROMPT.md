...
-   **Secrets in Code:** No secrets (API keys, passwords) are hardcoded. The code correctly references `os.environ` for secrets like `SESSION_SECRET` and `DA
...
- **What to change:** Implement the scoring formula in `nostr_monitor.py` (at ingestion time) and/or as a `@property` on `NostrMonitorEvent`. Ensure it matches L
