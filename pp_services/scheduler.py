    ...
    try:
        ...
        with app.app_context():
            ...
            if ENABLE_LIVE_POSTING and reel_url:
                ...
                try:
                    from pp_services.x_service import XService, can_post_tweet
                    x = XService()
...
def run_task(name: str) -> Dict:
    ...
    if name == "media_ai_summaries":
        try:
            from pp_services.media_feed_service import generate_ai_summaries
    ...
    if name == "mining_snapshot_hourly":
        try:
            from app import app
            from pp_services.mining_risk_service import snapshot_all
            with app.app_context():
                out = snapshot_all()
    ...
    if name == "sarah_intelligence_briefing":
        try:
            from pp_services.briefing_engine import briefing_engine
    ...
    if name == "sentiment_buffer_update":
        try:
            from pp_services.sentiment_service import sentiment_service
    ...
    if name == "monetization_injector":
        try:
            from app import app
            from pp_services.monetization_engine import monetization_engine
            with app.app_context():
    ...
    if name == "pulse_drop_rebuild_5am":
        try:
            from app import app
            from pp_services.channel_monitor import channel_monitor_service
            from pp_services.highlight_extractor import highlight_extractor_service
            from pp_services.commentary_generator import commentary_generator_service
            with app.app_context():
    ...
    if name in ("affiliate_education_morning", "affiliate_education_evening"):
        try:
            from app import app
            from pp_services.affiliate_article_generator import affiliate_article_generator
            with app.app_context():
    ...
    if name == "afternoon_article_tweet":
        try:
            from pp_services.x_daily_top_article import main as top_article_main
            top_article_main()
    ...
    if name in ("auto_engagement_noon", "auto_engagement_evening"):
        try:
            from app import app
            from pp_services.x_engagement_engine import run_auto_engagement
            with app.app_context():
    ...
    if name == "btc_milestone_check":
        try:
            from app import app
            from pp_services.price_service import PriceService
            from pp_services.milestone_service import milestone_service
            with app.app_context():
    ...
    if name == "weekly_performance_analysis":
        try:
            from app import app
            from pp_services.milestone_service import run_weekly_performance_analysis
            with app.app_context():
    ...
    if name in ("stage_brief_morning", "stage_brief_midday", "stage_brief_evening"):
        ...
        try:
            from pp_services.stage_brief_pipeline import generate_brief
    ...
    if name == "panopticon_congress_refresh":
        try:
            from pp_services.panopticon_service import fetch_stock_act_disclosures, _cache
    ...
    if name == "panopticon_whale_scan":
        try:
            from pp_services.panopticon_service import fetch_whale_alerts, _cache
    ...
    if name == "panopticon_polymarket_refresh":
        try:
            from pp_services.panopticon_service import fetch_polymarket_markets, _cache
    ...
    if name in ("pulse_render_morning", "pulse_render_midday", "pulse_render_afternoon"):
        try:
            from pp_services.gpu_scheduler import get_scheduler, run_episode_render
            sched = get_scheduler()
            episode_label = {
    ...
    if name == "gpu_health_monitor":
        try:
            from pp_services.gpu_scheduler import get_scheduler
            sched = get_scheduler()
            health = sched.status()
...
def initialize_scheduler() -> Dict:
    ...
    with _scheduler_lock:
        ...
        try:
            from pp_services.media_feed_service import sync_all_feeds, generate_ai_summaries
        ...
        try:
            from pp_services.gpu_scheduler import get_scheduler as _get_gpu_sched
