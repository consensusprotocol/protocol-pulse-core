...
from pp_services.youtube_service import YouTubeService  # type: ignore
...
def is_topic_similar_to_recent(topic: str, hours: int = 48) -> bool:
    ...
    try:
        from pp_services.content_generator import is_topic_duplicate_via_gemini
...
def generate_article_with_tracking(force: bool = False) -> dict:
    ...
    """
    with app_context():
        from pp_services.content_generator import auto_publish_enabled, validate_article_for_publish
        ...
        try:
            from pp_services.content_governor import get_next_assignment, record_published, SLOT_CATEGORY
        ...
        try:
            from pp_services.article_automation import RealNewsArticleGenerator
            from pp_services.article_automation import _sanitize_title, _is_banned_template
...
def generate_breaking_article_with_tracking() -> dict:
    ...
    with app_context():
        ...
        if not run:
            return {"skipped": True, "message": "Another process is running"}
        try:
            from pp_services.content_generator import (
                ContentGenerator,
...
def generate_podcasts_from_partners() -> dict:
    ...
    """
    from pp_services.youtube_service import youtube_service  # lazy import singleton if present
