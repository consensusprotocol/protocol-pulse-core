...
class DailyDriver:
    ...
    def _stage_source_ingestion(self):
        ...
        from pp_services.video_engine.sources.tweet_monitor import TweetMonitor
        from pp_services.video_engine.sources.market_data import MarketDataProvider
        from pp_services.video_engine.sources.spaces_monitor import fetch_recent_spaces
        from pp_services.video_engine.sources.bundle_assembler import SourceBundleAssembler
        ...
        nostr_posts = []
        try:
            from pp_services.video_engine.sources.nostr_capture import fetch_notable_nostr
        ...
        spaces_data = []
        try:
            from pp_services.video_engine.sources.spaces_listener import find_recent_spaces as find_spaces
    ...
    def _stage_grok_triage(self, source_bundle):
        ...
        from pp_services.video_engine.editorial.grok_triage import GrokTriage

        triage = GrokTriage()
        if not triage.configured:
            logger.warning("  Grok not configured — skipping triage")
            from pp_services.video_engine.editorial.schemas import TriageOutput
    ...
    def _stage_clustering(self, triage):
        ...
        if not triage or not triage.candidates:
            from pp_services.video_engine.editorial.schemas import ClusterOutput
            return ClusterOutput(clusters=[])

        from pp_services.video_engine.editorial.clustering import StoryClustering
    ...
    def _stage_claude_director(self, clusters, source_bundle):
        ...
        from pp_services.video_engine.editorial.claude_director import ClaudeDirector
    ...
    def _stage_qa_review(self, show_plan, triage):
        ...
        from pp_services.video_engine.editorial.qa_reviewer import QAReviewer
    ...
    def _stage_clip_extraction(self, show_plan, transcripts):
        ...
        from pp_services.video_engine.editorial.clip_extractor import ClipExtractor
    ...
    def _stage_narration(self, show_plan):
        ...
        from pp_services.video_engine.editorial.narration_generator import NarrationGenerator
    ...
    def _stage_manifest(self, show_plan, audio_map, clip_map):
        ...
        from pp_services.video_engine.assembly.manifest_builder import (
    ...
    def _stage_assembly(self, manifest, shorts_manifests,
        ...
        from pp_services.video_engine.ultron_client import is_local_mode

        if is_local_mode() and show_plan and audio_map is not None:
            # Use EpisodeBuilder for full local assembly
            from pp_services.video_engine.assembly.episode_builder import EpisodeBuilder
            ...
            # Build shorts
            if shorts_manifests:
                from pp_services.video_engine.assembly.shorts_builder import ShortsBuilder
            ...
            # Build teaser
            try:
                from pp_services.video_engine.assembly.teaser_builder import build_teaser
        ...
        from pp_services.video_engine.assembly.ultron_assembler import UltronAssembler
    ...
    def _stage_post_qa(self):
        ...
        from pp_services.video_engine.assembly.post_assembly_qa import PostAssemblyQA
    ...
    def _stage_distribution_pack(self, show_plan):
        ...
        from pp_services.video_engine.assembly.distribution_pack import DistributionPackGenerator
    ...
    def _stage_avatar(self, audio_map):
        ...
        try:
            from pp_services.video_engine.editorial.avatar_generator import AvatarGenerator
    ...
    def _send_alerts(self, result):
        ...
        try:
            from pp_services.video_engine.pipeline_alerts import (
