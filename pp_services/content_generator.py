...
from pp_services.x_service import get_social_feedback
from services import x_service
from pp_services.image_service import image_service
from pp_services.gemini_service import gemini_service
from pp_services.node_service import NodeService
from pp_services.fact_checker import fact_checker, verify_article_before_publish
...
def get_article_header_url(title: str, article_html: str = "") -> str:
    """
...
def auto_publish_enabled() -> bool:
    """Check if auto-publishing is enabled. Defaults to True.
    Set DISABLE_AUTO_PUBLISH=true to turn off, or ENABLE_AUTO_PUBLISH=false."""
    if os.environ.get("DISABLE_AUTO_PUBLISH", "").lower() in ("true", "1", "yes"):
        return False
    if os.environ.get("ENABLE_AUTO_PUBLISH", "").lower() in ("false", "0", "no"):
        return False
    return True


def validate_article_for_publish(article) -> tuple:
    """
    Validate an article before publishing.
    Returns (ok: bool, errors: list[str]).
    """
    errors = []
    title = getattr(article, 'title', None) or (article.get('title') if isinstance(article, dict) else None) or ''
    content = getattr(article, 'content', None) or (article.get('content') if isinstance(article, dict) else None) or ''

    if not title or len(title.strip()) < 10:
        errors.append("Title too short (min 10 chars)")
    if not content or len(content.strip()) < 200:
        errors.append("Content too short (min 200 chars)")
    if content and len(content.split()) < 50:
        errors.append("Content has fewer than 50 words")

    return (len(errors) == 0, errors)


def should_article_be_draft_by_word_count(content: str, min_words: int = 100) -> bool:
    """Return True if the article is too short to auto-publish."""
    if not content:
        return True
