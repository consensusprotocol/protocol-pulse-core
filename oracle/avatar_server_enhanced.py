...
AVATAR_SOURCE = os.path.join(os.path.dirname(__file__), "avatar_source.png")
...
BLINK_DURATION = 0.18      # seconds for a full blink (close + open)
HEAD_ROTATION_AMPLITUDE = 0.8  # degrees max rotation
...
def wav2lip_generate(audio_path):
    ...
    for batch_start in range(0, total_chunks, BATCH_SIZE):
        ...
        img_batch = np.array([face_resized / 255.0] * len(batch_mels), dtype=np.float32)
        mel_batch = np.array([m.T for m in batch_mels], dtype=np.float32)
