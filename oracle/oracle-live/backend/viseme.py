...
def chars_to_word_spans(alignment: dict) -> List[Tuple[str, float, float]]:
    ...
    if not chars or not starts or not ends or not (len(chars) == len(starts) == len(ends)):
        return []
    ...
    for idx, (ch, st, en) in enumerate(zip(chars, starts, ends)):
        ...
        current.append(ch)
        if is_last:
            spans.append(("".join(current), float(current_start), float(en)))
    return spans
...
def phonemes_to_visemes(phones: List[str]) -> List[int]:
    ...
    for p in phones:
        base = p.rstrip("012")
        out.append(PHONEME_TO_VISEME.get(base, 10))
    return out or [0]

def _duration_weight_for_phone(phone: str) -> float:
    base = phone.rstrip("012")
    if base in {"AA","AH","AE","EH","IH","IY","AO","OW","UW","UH","AY","OY","AW","EY"}:
        return 1.55
    if base in {"M","N","NG","R","L"}:
        return 1.15
    if base in {"S","Z","SH","ZH","F","V","TH","DH"}:
        return 1.2
    return 0.95
...
def build_viseme_timeline(text: str, alignment: dict) -> List[dict]:
    ...
    for raw_word, start_t, end_t in spans:
        ...
        weights = [_duration_weight_for_phone(p) for p in phones]
        ...
        for i, (phone, viseme_id, w) in enumerate(zip(phones, visemes, weights)):
            ...
            timeline.append({
                ...
                "duration": round(max(0.018, dur), 4),
                "word": word,
            })
            cursor += dur
    ...
    if coalesced:
        end_time = coalesced[-1]["time"] + coalesced[-1]["duration"]
    else:
        end_time = 0.0
