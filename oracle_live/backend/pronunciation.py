...
COMMON_WORDS: Dict[str, List[str]] = {
    ...
    "price": ["P","R","AY","S"], "market": ["M","AA","R","K","AH","T"],
    "today": ["T","AH","D","EY"],
}
    ...
    while i < len(word):
        ...
        for grapheme, phones in rules:
            if word[i:i+len(grapheme)] == grapheme:
                out.extend(phones); i += len(grapheme); matched = True; break
        if matched: continue
        out.extend(singles.get(word[i], [])); i += 1
    return out or ["AH"]

def lookup_pronunciation(word: str) -> List[str]:
    w = normalize_word(word)
    if not w: return ["AH"]
    if w in DOMAIN_LEXICON: return DOMAIN_LEXICON[w]
    if w in COMMON_WORDS: return COMMON_WORDS[w]
    if w.isdigit():
        dm = {"0":["Z","IY","R","OW"],"1":["W","AH","N"],"2":["T","UW"],
              "3":["TH","R","IY"],"4":["F","AO","R"],"5":["F","AY","V"],
              "6":["S","IH","K","S"],"7":["S","EH","V","AH","N"],
              "8":["EY","T"],"9":["N","AY","N"]}
        out = []
        for ch in w: out.extend(dm.get(ch,[]))
        return out or ["AH"]
