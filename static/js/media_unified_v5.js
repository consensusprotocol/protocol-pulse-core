...
// ─── BECH32→HEX CONVERTER (for Nostr REQ authors filter) ──────────
function bech32ToHex(str) {
  ...
  let vals = [];
  for (const c of data) { const i = ALPHA.indexOf(c); if (i !== -1) vals.push(i); }
  vals = vals.slice(0, -6); // strip checksum
  let bits = '', hex = '';
  for (const v of vals) bits += v.toString(2).padStart(5, '0');
  ...
  connect(url) {
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        // Convert npub bech32 → hex for valid Nostr REQ authors filter
        const hexPubkeys = NOSTR_PUBKEYS.map(npub => bech32ToHex(npub)).filter(h => h.length === 64);
...
function updateSignalStrength() {
  ...
  // Also update signal gauge section elements
  const sigComposite  = document.getElementById('sig-composite');
  const sigSentiment  = document.getElementById('sig-sentiment');
  const sigSpaces     = document.getElementById('sig-spaces');
  if (sigComposite) sigComposite.textContent = state.signalScore;
  if (sigSentiment) sigSentiment.textContent = Math.round(sentimentScore);
  if (sigSpaces)    sigSpaces.textContent    = '--';
