...
def _epx_confidence(features):
    filled = sum(1 for v in features.values() if v is not None and v != 0)
    total = len(features)
    return round(min(0.92, max(0.30, filled / total)), 2)
...
def _ihx_confidence(features):
    filled = sum(1 for v in features.values() if v is not None and v != 0)
    total = len(features)
    return round(min(0.88, max(0.25, filled / total)), 2)
        ...
        with app.app_context():
            for r in results:
                entry = SNModel(
                    ...
                    freshness_seconds=int((now - datetime.utcnow()).total_seconds()),
...
def compute_convergence(signals_list: List[dict]) -> dict:
    ...
    # Participation rate
    participation = total / 6  # 6 expected signals eventually
